...
def run_groundingdino_map(List,confidence,alpha):
    caption = ". ".join(List)
    ...
    # 保存预测结果和mAP计算（保持不变）
    pred_file = "groundingdino_clip_confidence-1.json"
    ...
    if predictions:
        ...
        print(f"confidence:{confidence},GroundingDINO+CLIP融合 mAP: {mAP:.5f}%")
        with open('groundeddino_clip_confidence-1.txt', 'a', encoding='utf-8') as f:
            f.write(f'confidence:{confidence},alpha:{alpha},mAP结果：{mAP:.7f} %\n')


if __name__ == "__main__":
    for confidence in np.arange(0.15, 0.5, 0.05):
        for alpha in np.arange(0, 1.2, 0.2):
            run_groundingdino_map(List,confidence,alpha)
